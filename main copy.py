...
class TSP:
    ...
    def best_path(self):
        ...
        print(best_path,' cost : ',min_cost)
        return(best_path,min_cost)
        

        

    def swap_mutation(self, mutation_operator):
            print('size of mutation', round(mutation_operator, 1) *len(self.populationList))
            for i in range(0,int(round(mutation_operator, 1)*len(self.populationList))):
                index=random.randint(0,len(self.populationList)-1)
                a = random.randint(0, len(self.populationList) - 1)
                b = random.randint(0, len(self.populationList) - 1)
                self.populationList[index][b], self.populationList[index][a] = self.populationList[index][a], self.populationList[index][b]
